...
MUT <- mut |> 
  ...
  arrange(gene,POS) |>
  dplyr::select(gene,Antiviral=drug_resistance,"Mutation (% échantillon portant la mutation)"=mutation,"Type de mutation"=drug_resistance_type) |>
  distinct()
...
profils <- mut |> 
  filter(!drug_resistance_type %in% c("-",NA,""),!drug_resistance %in% c("-",NA,"")) |> 
  group_by(drug_resistance_type) |> 
  distinct(drug_resistance) |>
  summarise(Antiviral=str_flatten_comma(drug_resistance))

Rprofil <- filter(profils,str_starts(drug_resistance_type,"R")) |> pull()
Sprofil <- filter(profils,str_starts(drug_resistance_type,"S")) |> pull()
if (is_empty(Rprofil)) Rprofil <- ""
if (is_empty(Sprofil)) Sprofil <- ""
doc <- body_replace_all_text(doc,"LISTERESISTANCES", Rprofil, fixed = TRUE)
doc <- body_replace_all_text(doc,"LISTESUSCEPTIBLES", Sprofil, fixed = TRUE) 
